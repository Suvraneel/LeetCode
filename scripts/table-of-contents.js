...
const homePage = (problems) => {
  ...
  const donut = `<section class="chart" aria-label="Problem difficulty distribution"><div class="donut" style="background:conic-gradient(var(--easy) 0 ${easyEnd}%,var(--medium) ${easyEnd}% ${mediumEnd}%,var(--hard) ${mediumEnd}% 100%)"><span><b>${problems.length}</b>solved</span></div><div class="chart-copy"><h2>Difficulty mix</h2><p><span class="easy">●</span> ${easy} Easy &nbsp; <span class="medium">●</span> ${medium} Medium &nbsp; <span class="hard">●</span> ${hard} Hard</p></div></section>`;
  ...
  return homePageBase(problems)
    ...
    .replace('</section><section><div class="controls">', `</section>${donut}<section><div class="controls">`)
    .replace('</main>', `${footer}</main>`)
    .replace('</style>', '.profile-handle{display:block;margin-top:.35rem;color:var(--link);font:600 .88rem ui-monospace,SFMono-Regular,monospace;letter-spacing:-.02em}.chart{display:flex;align-items:center;gap:1rem;margin:1.25rem 0;padding:1rem 0;border-bottom:1px solid var(--line)}.donut{width:92px;height:92px;border-radius:50%;display:grid;place-items:center;flex:0 0 auto}.donut span{display:grid;place-items:center;width:62px;height:62px;border-radius:50%;background:var(--bg);color:var(--muted);font-size:.72rem;text-align:center}.donut b{display:block;color:var(--text);font-size:1.1rem}.chart-copy h2{font-size:1rem;margin:0}.chart-copy p{margin:.25rem 0 0;font-size:.9rem}footer{border-top:1px solid var(--line);margin-top:3rem;padding-top:1.25rem;color:var(--muted);font-size:.9rem}footer p{margin:0 0 .55rem}.social-links{display:flex;gap:.8rem;margin-bottom:.65rem}.social-links a{display:grid;place-items:center;width:2rem;height:2rem;border:1px solid var(--line);border-radius:7px}.social-links img{width:1rem;height:1rem}.social-links a:hover{border-color:var(--link);transform:translateY(-1px)}footer a{color:var(--link);text-decoration:none}.star-link{display:block;margin-bottom:.55rem}footer small{font-size:.8rem}@media(max-width:650px){.chart{margin:.8rem 0}}</style>');
};
